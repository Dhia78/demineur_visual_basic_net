Public Class Grille4
    ...
    Private Sub Grille_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        For Each p As Integer In TabMines
            'haut'
            If p = 666 And cpt > 4 Then
                If TabMines(cpt - 4) <> 666 Then
                    TabMines(cpt - 4) += 1
                End If
            End If
            'bas'
            If p = 666 And cpt < 12 Then
                If TabMines(cpt + 4) <> 666 Then
                    TabMines(cpt + 4) += 1
                End If
            End If
            'gauche'
            If p = 666 And cpt Mod 4 <> 0 Then
                If TabMines(cpt - 1) <> 666 Then
                    TabMines(cpt - 1) += 1
                End If
            End If
            'droite'
            If p = 666 And (cpt + 1) Mod 4 <> 0 Then
                If TabMines(cpt + 1) <> 666 Then
                    TabMines(cpt + 1) += 1
                End If
            End If
            'haut-gauche'
            If p = 666 And cpt > 4 And cpt Mod 4 <> 0 Then
                If TabMines(cpt - 5) <> 666 Then
                    TabMines(cpt - 5) += 1
                End If
            End If
            'haut-droit'
            If p = 666 And cpt > 4 And (cpt + 1) Mod 4 <> 0 Then
                If TabMines(cpt - 3) <> 666 Then
                    TabMines(cpt - 3) += 1
                End If
            End If
            'bas-gauche'
            If p = 666 And cpt < 12 And cpt Mod 4 <> 0 Then
                If TabMines(cpt + 3) <> 666 Then
                    TabMines(cpt + 3) += 1
                End If
            End If
            'bas-droit'
            If p = 666 And cpt < 12 And (cpt + 1) Mod 4 <> 0 Then
                If TabMines(cpt + 5) <> 666 Then
                    TabMines(cpt + 5) += 1
                End If
            End If
    ...
    Private Sub Demasquer(sender As Object, e As EventArgs)
        ...
        If djMine.Contains(indBtn) Then
            ...
            For i As Integer = 0 To 3
                For j As Integer = 0 To 3
                    If TabMines(i * 4 + j) = 666 Then
                        PanelGrille.Controls.Item(i * 4 + j).BackColor = Color.Red
                    End If
                Next
            Next
        ...
        ElseIf TabMines(indBtn) = 0 Then
            ...
            If indBtn > 4 And indBtn Mod 4 <> 0 And (indBtn + 1) Mod 4 <> 0 And indBtn < 12 Then

                Dim haut = PanelGrille.GetControlFromPosition(col, (row / 4) - 1)
                Dim bas = PanelGrille.GetControlFromPosition(col, (row / 4) + 1)
                Dim gauche = PanelGrille.GetControlFromPosition(col - 1, row / 4)
                Dim droite = PanelGrille.GetControlFromPosition(col + 1, row / 4)
                Dim hg = PanelGrille.GetControlFromPosition(col - 1, (row / 4) - 1)
                Dim hd = PanelGrille.GetControlFromPosition(col + 1, (row / 4) - 1)
                Dim bg = PanelGrille.GetControlFromPosition(col - 1, (row / 4) + 1)
                Dim bd = PanelGrille.GetControlFromPosition(col + 1, (row / 4) + 1)


                If TabMines(indBtn - 4) = 0 Then
                    haut.Text = ""
                Else
                    haut.Text = TabMines(indBtn - 4)
                ...
                If TabMines(indBtn - 5) = 0 Then
                    hg.Text = ""
                Else
                    hg.Text = TabMines(indBtn - 5)
                ...
                If TabMines(indBtn - 3) = 0 Then
                    hd.Text = ""
                Else
                    hd.Text = TabMines(indBtn - 3)
            ...
            If indBtn Mod 4 = 0 And indBtn < 12 And indBtn > 3 Then
            ...
            If (indBtn + 1) Mod 4 = 0 And indBtn < 12 And indBtn > 3 Then
